...
public sealed class DifferenceCalculator
{
    /// <summary>
    ...
    /// <returns></returns>
    public string? GetDelta<T>(T oldInstance, T newInstance) where T : class
    {
        var oldDocument = GetJsonDocument(oldInstance).RootElement;
        var newDocument = GetJsonDocument(newInstance).RootElement;
    ...
    /// <returns></returns>
    private JsonDocument GetJsonDocument(object element) 
