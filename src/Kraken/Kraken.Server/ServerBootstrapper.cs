...
internal class ServerBootstrapper : BackgroundService
{
    ...
    {
        _logger.LogInformation("<<<<<<<<<< Kraken background task is starting.>>>>>>>>>>");
        try
        {
        ...
        _cts.Token.Register(() =>
        {
            _logger.LogInformation("<========= Kraken background task is stopping.");
        ...
        _logger.LogInformation(kraken_logo);
    }
    ...
    private const string kraken_logo = @"

                                Kraken Framework - Powered by Dottex
